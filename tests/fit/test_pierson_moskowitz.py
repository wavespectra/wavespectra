...
def test_hs_fp(freq):
    ...
    ds = fit_pierson_moskowitz(freq=freq, fp=0.1, hs=2)
...
def test_jonswap_gamma_1_equal(freq):
    ...
    ds2 = fit_pierson_moskowitz(freq=freq, fp=0.1, hs=2)
...
def test_freq_input_type(freq):
    ...
    ds1 = fit_pierson_moskowitz(freq=freq, fp=0.1, hs=2)
    ds2 = fit_pierson_moskowitz(freq=freq.values, fp=0.1, hs=2)
    ds3 = fit_pierson_moskowitz(freq=list(freq.values), fp=0.1, hs=2)

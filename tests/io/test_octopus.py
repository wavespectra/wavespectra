...
class TestOctopus(object):
    ...
    def setup_class(self):
        ...
        self.example2 = read_octopus(os.path.join(FILES_DIR, "Argoss_example.csv"))
        self.example3 = read_octopus(os.path.join(FILES_DIR, "octopusfile.oct"))
    ...
    def test_hs(self):
        assert self.example1.spec.hs().values == pytest.approx(
            EXAMPLE_HS, rel=0.01
        )

    def test_hs_reported(self):
        assert self.example1.Reported_Hs.values == pytest.approx(
            EXAMPLE_HS, abs=0.2
        )

    def test_TP(self):
        assert self.example1.spec.tp().values == pytest.approx(
            EXAMPLE_TP, rel=0.01
        )

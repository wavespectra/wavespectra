...
from wavespectra.construct.frequency import jonswap, tma
...
def test_hs_tp(freq):
    ...
    ds1 = tma(freq=freq, fp=0.1, dep=10, hs=2)
    ds2 = tma(freq=freq, fp=0.1, dep=50, hs=2)
...
def test_jonswap_tma_deepwater_equal(freq):
    ...
    ds1 = jonswap(freq=freq, fp=0.1, hs=2)
    ds2 = tma(freq=freq, fp=0.1, dep=80, hs=2)
...
def test_freq_input_type(freq):
    ...
    ds1 = tma(freq=freq, fp=0.1, dep=10.0, hs=2)
    ds2 = tma(freq=freq.values, fp=0.1, dep=10.0, hs=2)
    ds3 = tma(freq=list(freq.values), fp=0.1, dep=10.0, hs=2)
...
def test_scaling(freq):
    ...
    ds1 = tma(freq=freq, fp=0.1, dep=10.0, gamma=1.0, alpha=0.0081, hs=2)
    ds2 = tma(freq=freq, fp=0.1, dep=10.0, gamma=3.3, alpha=0.0081)
